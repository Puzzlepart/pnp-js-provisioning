    ...
    processView(web, lc, lvc) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                yield this.processViewFields(view, lvc.ViewFields);
            }
            catch (err) {
                const result = yield web.lists.getByTitle(lc.Title).views.add(lvc.Title, lvc.PersonalView, lvc.AdditionalSettings);
                sp_pnp_js_1.Logger.log({ level: sp_pnp_js_1.LogLevel.Info, message: `View ${lvc.Title} added successfully to list ${lc.Title}.` });
                yield this.processViewFields(result.view, lvc.ViewFields);
            }
        });
    }
    /**
