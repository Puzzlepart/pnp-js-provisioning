...
class Lists extends handlerbase_1.HandlerBase {
    ...
    processField(web, lc, fieldXml) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            sp_pnp_js_1.Logger.log({ message: `Processing field ${internalName} (${displayName}) for list ${lc.Title}.`, level: sp_pnp_js_1.LogLevel.Info });
            ...
            try {
                ...
                yield field.delete();
                sp_pnp_js_1.Logger.log({ message: `Field ${internalName} (${displayName}) successfully deleted from list ${lc.Title}.`, level: sp_pnp_js_1.LogLevel.Info });
            }
            catch (err) {
                sp_pnp_js_1.Logger.log({ message: `Field does not exist ${internalName} (${displayName}) in list ${lc.Title}.`, level: sp_pnp_js_1.LogLevel.Info });
            }
            // Looks like e.g. lookup fields can't be updated, so we'll need to re-create the field
            try {
                ...
                sp_pnp_js_1.Logger.log({ message: `Failed to add field '${displayName}' to list ${lc.Title}.`, level: sp_pnp_js_1.LogLevel.Warning });
            }
        });
    }
    /**
